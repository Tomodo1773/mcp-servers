...
// GitHub logins permitted to use this MCP server (stored lowercase)
const ALLOWED_USERS: Set<string> = new Set(["tomodo1773"]);
...
app.get("/callback", async (c) => {
  ...
  if (!ALLOWED_USERS.has(login.toLowerCase())) {
    return c.text(
      "Forbidden: Your GitHub account is not authorized to use this service.",
      403,
    );
  }
