...
async function withAuth<T>(
  ...
  let token = await getValidToken(env, userId);
  try {
    return await fn(token, userId);
  } catch (err) {
    if (err instanceof SpotifyApiError && err.status === 401) {
      token = await refreshAccessToken(env, userId);
      return await fn(token, userId);
    }
    throw err;
  }
...
async function handleTool(
  ...
  try {
    const text = await withAuth(env, fn);
...
function registerTools(server: McpServer, env: Env): void {
  ...
  server.tool(
    ...
    async ({ item_uri }) =>
      handleTool(env, async (token, userId) => {
...
export function createSpotifyMcpHandler(): {
  fetch: (
    request: Request,
    env: Env,
